...
pub fn extract_audio_links(page_link: &str) -> Result<Vec<(String, TrackMetadata)>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    for (name, value) in &element.value().attrs {
        if &name.local == "data-tralbum" {
            ...
            links = data
                ...
                .array()
                .iter()
                .map(|f| (f.get("file.mp3-128").to_string(), 
                          TrackMetadata{name: f.get("title").to_string(), track_number: u16::from_str_radix(&f.get("track_num").to_string(), 10).unwrap(), artist: artist.to_owned(), album: album.to_owned() }))
